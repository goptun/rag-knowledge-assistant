...
> **Nota operacional (deploy de 2026-09-20).** O contêiner `rag_api` está com o `rate_limit.py` novo aplicado por `docker cp` + `docker restart`, e **não** por rebuild, de propósito: (1) os modelos do embedder/reranker (~1,5 GB) ficam na camada gravável do contêiner (não estão na imagem nem em volume), então *recriar* o contêiner obriga a baixar tudo de novo do Hugging Face na subida; (2) o cache de build foi invalidado — o tag `python:3.12-slim` avançou de digest — e um `up --build` reinstalaria `torch` (~90 min de CPU na VPS de 2 núcleos). Enquanto o contêiner não for recriado, o código nele já é o do `main`. Para tornar isso robusto, o próximo passo é persistir `/root/.cache/huggingface` num volume nomeado e fixar o digest da imagem base; só então vale recriar o contêiner. Rollback do limiter: `docker cp ~/rag_rate_limit.py.orig rag_api:/app/app/api/rate_limit.py && docker restart rag_api`.
